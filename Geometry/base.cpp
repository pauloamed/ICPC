...
// funcao pra checar se poligono eh convexo
// assume que p eh dado com os pontos ordenados
bool isConvex(vector<pv> p){
    bool hasPos = false, hasNeg = false;
    for(int i = 0; i < p.size(); ++i){
        int temp = p[i].orient(p[(i+1)%p.size()], p[(i+2)%p.size()]); // checnado atual e dois prox
        if(temp < 0) hasNeg = true; // se os pontos sao dados em sentido horario
        if(temp > 0) hasPos = true; // se os pontos sao dados em sentido antihorario
    }
    return !(hasNeg && hasPos); // se tem horario e antihorario ta ruim
}
